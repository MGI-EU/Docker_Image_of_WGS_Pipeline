...
=head1 Usage
	perl $0 [option] <bam> <outdir>
		...
		-hg	hg19 or hg18 or mm9 default:hg19
		-h	help
...
my ($basethres,$mapQthres,$total_chr,$help,$hg);
GetOptions("q:i"=>\$basethres,"Q:i"=>\$mapQthres,"l:i"=>\$total_chr,"hg:s"=>\$hg,"h"=>\$help);
...
my $bam2depth="/opt/pipeline/bin/bam2depth";

if($hg =~ /hg19/i){
	open DEPTH,"$bam2depth -q $basethres -Q $mapQthres $bam -b /opt/pipeline/database/reference/hg19/hg19.nonN_region | " or die;
}
elsif($hg =~ /mm9/i)
{
