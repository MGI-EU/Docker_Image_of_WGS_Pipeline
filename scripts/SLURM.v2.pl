...
=Usage
  ...
  --submit <qsub/sbatch/sh> set the command for job submitting. default "sh"
  --cmd <string>    set parameters for qsub/sbatch. default[]
...
my ($job_cmd,$cmd,$Interval,$Lines, $Maxjob, $Help);
GetOptions(
	"submit:s"=>$job_cmd,
	"cmd:s"=>$cmd,
	"lines:i"=>\$Lines,
